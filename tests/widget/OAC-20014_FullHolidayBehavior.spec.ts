...
test.describe('Full Holiday Behavior - OAC-20014', () => {
  ...
  //   // Create full holiday payload
  //   holidayData = {
  //     HolidayId: -88,
  //     FkHolidayParentId: null,
  //     FkLocationId: 1,
  //     HolidayName: `Test Holiday ${isoDate.split('T')[0]}`,
  //     HolidayCode: "X",
  //     Date: isoDate,
  //     StartTime: isoDate,
  //     EndTime: isoDate,
  ...
  //   console.log(`Created full holiday for date: ${isoDate.split('T')[0]}`);
  // });

  // test.afterAll(async ({ request }) => {
  //   // Cleanup: Delete the created holiday
  //   try {
  //     // First, retrieve the holiday to get its proper ID
  //     const year = holidayDate.getFullYear();
  //     const getResponse = await adminService.getAllHolidays(year, 54, 1);
      
  //     if (getResponse.ok()) {
  //       const holidays = await getResponse.json();
  //       const createdHoliday = holidays.value?.find((h: any) => 
  //         h.HolidayName === holidayData.HolidayName && 
  //         h.Date === holidayData.Date
  //       );

  //       if (createdHoliday) {
  //         // Delete the holiday using the retrieved data
  //         const deleteResponse = await adminService.deleteHoliday(createdHoliday, 54);
  //         if (deleteResponse.ok()) {
  //           console.log(`Successfully deleted holiday: ${holidayData.HolidayName}`);
  //         } else {
  //           console.error(`Failed to delete holiday: ${holidayData.HolidayName}`);
  //         }
  //       } else {
  //         console.warn(`Holiday not found for deletion: ${holidayData.HolidayName}`);
  //       }
  //     }
  //   } catch (error) {
  //     console.error('Error during holiday cleanup:', error);
  //   }
  // });
