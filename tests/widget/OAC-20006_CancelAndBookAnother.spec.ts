...
/**
 * OAC-20006: Book Another From Cancellation Page (Desktop, Functional)
 * 
 * Test Case: Verify after cancellation, Book Another keeps personal details and completes a new booking.
 * 
 * Prerequisites:
 ...
 * - Cancelable appointment exists
 * - Personal details persistence enabled in widget
 * 
 * Test Flow:
 * 1. Create first appointment and capture details
 * 2. Verify cancel button availability
 * 3. Click Cancel and confirm cancellation
 * 4. Verify cancellation messages and Book Another shown
 * 5. Click Book Another to start new booking
 * 6. Select different service, date, and time for second booking
 * 7. Verify personal details are pre-filled on personal details page
 * 8. Submit second booking and verify confirmation
 * 9. Validate personal details consistency across both appointments
 * 
 * Expected Results:
 * - New booking created with retained personal details
 * - Cancellation flow works correctly
 * - Personal details persisted across multiple bookings
 */
test.describe('Appointment Booking - OAC-20006', () => {
  test('Book Another From Cancellation Page retains personal details and allows second booking', { tag: ['@functional', '@smoke'] }, async ({ page, secondBookingData, secondServiceData }) => {
    ...
    // Step 3: Verify Cancel action availability
    await expect(confirmationPage.cancelButton).toBeVisible();

    // Step 4: Click Cancel and confirm cancellation
    const cancelSuccessful = await confirmationPage.testCancelAppointment();
    expect(cancelSuccessful).toBe(true);

    // Step 5: Verify cancellation messages and Book Another shown
    await confirmationPage.waitForCancellationConfirmation();
    const isAppointmentCancelled = await confirmationPage.verifyAppointmentCancelled();
    expect(isAppointmentCancelled).toBe(true);
    
    // Verify Book Another button is visible after cancellation
    await expect(confirmationPage.bookAnotherButton).toBeVisible();

    // Step 6: Click Book Another to start new booking
    ...
    // Step 7: Select different service for second booking
    ...
    // Step 8: Select different date/time for second booking
    ...
    // Step 9: Check personal details page - should be pre-filled
    await personalDetailsPage.waitForPersonalDetailsPage();

    // Step 10: Verify all details are retained correctly
    ...
    // Step 11: Submit second booking
    await personalDetailsPage.submit();

    // Step 12: Verify second confirmation
    await confirmationPage.waitForConfirmationPage();
    ...
    // Step 13: Verify personal details consistency across both appointments
    const secondConfirmationDetails = await confirmationPage.getConfirmationDetails();
    expect(secondConfirmationDetails.customerName).toBe(firstConfirmationDetails.customerName);
    expect(secondConfirmationDetails.customerEmail).toBe(firstConfirmationDetails.customerEmail);
    expect(secondConfirmationDetails.customerPhone).toBe(firstConfirmationDetails.customerPhone);
